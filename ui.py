...
def create_streamlit_app():
    ...
    # Tab 2: URL Scraper and Kafka Streamer
    with tab2:
        st.title("🔗 URL Scraper and Kafka Streamer")
        st.markdown("Process web content and stream documents to Kafka for real-time indexing")
        ...
        with st.form("url_processor_form", clear_on_submit=False):
            ...
            url_input = st.text_input(
                ...
                help="Enter the full URL to scrape and process"
            )
            
            # Options in columns
            col1, col2 = st.columns([2, 1])
            with col1:
                st.info("ℹ️ Using advanced web scraper for content extraction")
            
            with col2:
                document_preview = st.checkbox(
                    "Preview Document",
                    value=True,
                    help="Show document preview after processing"
                )
        ...
        if submit_btn:
            if not url_input:
                ...
                with st.status("Processing URL...", expanded=True) as status:
                    st.write("🔍 Scraping content from URL...")
                    st.write("📄 Extracting markdown content...")
                    st.write("📤 Streaming to Kafka...")
                    
                    try:
                        # Process URL (no chunker_type needed anymore)
                        ...
                        if not result or not result.get("success"):
                            ...
                            col1.metric("📄 Document Created", "1")
                            col2.metric("🔧 Processing Type", "Web Scraping")
                            col3.metric("📊 Total Words", document.get("word_count", 0))
                            
                            # Store in database (update to store as scraper type)
                            if store_processed_url(url_input, "scraper", 1):
                                st.success("✅ URL information stored in database")
                            
                            # Show document preview if requested
                            if document_preview and document:
                                st.markdown("### 📋 Document Preview")
                                
                                # Document metadata
                                metadata_cols = st.columns(4)
                                metadata_cols[0].metric("Document ID", document.get("document_id", "N/A")[:8] + "...")
                                metadata_cols[1].metric("Word Count", document.get("word_count", 0))
                                metadata_cols[2].metric("Processed At", document.get("timestamp", "N/A")[:19])
                                metadata_cols[3].metric("Status", "✅ Sent to Kafka")
                                ...
                                st.success("🎉 Document successfully processed and sent to Kafka!")
                            
                            elif not document:
                                st.warning("⚠️ No document was created from the processed URL")
